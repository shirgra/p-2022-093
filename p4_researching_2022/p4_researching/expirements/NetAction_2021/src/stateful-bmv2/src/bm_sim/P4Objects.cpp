...
P4Objects::process_single_param(ActionFn* action_fn,
  ...
  } else if (type == "register_stack") {
    const auto name = cfg_parameter["value"].asString();
    auto register_stack = get_register_stack_cfg(name);
    action_fn->parameter_push_back_register_stack(register_stack);
  } else if (type == "header_stack") {
...
P4Objects::init_meter_arrays(const Json::Value &cfg_root,
  ...
  for (const auto &cfg_meter_array : cfg_meter_arrays) {
    ...
    if (cfg_meter_array["is_ewma"].asBool() &&
          cfg_meter_array.isMember("window_size")) {
      const uint64_t window_size = cfg_meter_array["window_size"].asUInt64();
      meter_array->set_ewma_rates(window_size);
    }
...
void
P4Objects::init_register_stacks(const Json::Value &cfg_root) {
  DupIdChecker dup_id_checker("register");
  const Json::Value &cfg_register_stacks = cfg_root["register_stacks"];
  for (const auto &cfg_register_stack : cfg_register_stacks) {
    const string name = cfg_register_stack["name"].asString();
    const p4object_id_t id = cfg_register_stack["id"].asInt();
    dup_id_checker.add(id);
    const size_t size = cfg_register_stack["size"].asUInt();
    const int bitwidth = cfg_register_stack["bitwidth"].asInt();

    auto *register_stack = new RegisterStack(name, id, size, bitwidth);
    add_register_stack(name, unique_ptr<RegisterStack>(register_stack));
  }
}
  ...
  for (const auto &cfg_pipeline : cfg_pipelines) {
    ...
    for (const auto &cfg_table : cfg_tables) {
      ...
      if (table_type == "stateful") {
        std::vector<MatchKeyBuilder> key_builders;
        const Json::Value cfg_match_keys = cfg_table["keys"];

        for (const auto &cfg_match_key : cfg_match_keys) {
          ...
          key_builders.push_back(key_builder);
        } // keys
        ...
        const string flow_ctx_hdr_type = cfg_table["flow_context_header_type"]
                                                              .asString();

        std::unique_ptr<StatefulTable> table;
        HeaderType *flow_ctx_ht = get_header_type_cfg(flow_ctx_hdr_type);
        uint64_t idle_timeout;

        if (with_ageing)
          idle_timeout = cfg_table["idle_timeout"].asUInt64();

        table = StatefulTable::create_stateful_table(
            table_name, table_id, table_size, key_builders,
            with_counters, with_ageing, idle_timeout, lookup_factory, flow_ctx_ht);

        if (with_ageing)
          ageing_monitor->add_stateful_table(table.get());
        ...
        // check for timers
        if (cfg_table.isMember("with_timers")) {
          const bool has_timers = cfg_table["with_timers"].asBool();
          if (has_timers) {
            const uint64_t timer_granularity = cfg_table["timer_granularity"].asUInt64();

            add_stateful_timer(new StatefulTimer(table_name, timer_granularity));
          }
        }

        add_stateful_table(table_name, std::move(table));
        continue;
      }
    ...
    for (const auto &cfg_table : cfg_tables) {
      ...
      MatchTableAbstract *table = table_type != "stateful" ?
                                  get_abstract_match_table(table_name) :
                                get_abstract_state_table(table_name);
...
P4Objects::reset_state() {
  ...
  for (const auto &e : stateful_tables_map) {
    e.second->get_match_table()->reset_state();
  }
...
P4Objects::id_from_name(ResourceType type, const std::string &name,
  ...
  switch (type) {
    ...
    case ResourceType::STATE_TABLE:
      return id_from_name_(stateful_tables_map, name, id);
...
P4Objects::get_abstract_match_table_rt(const std::string &name) const {
  auto it = match_action_tables_map.find(name);
    if (it == match_action_tables_map.end()) {
        auto itt = stateful_tables_map.find(name);
        if (itt != stateful_tables_map.end()) {
            return itt->second->get_match_table();
        } else return nullptr;
    } else {
        return it->second->get_match_table();
    }
}
...
void
P4Objects::add_stateful_table(const std::string &name,
                                  std::unique_ptr<StatefulTable> table) {
  add_control_node(name, table.get());
  // not really needed as add_control_node enforces a stricter check
  add_new_object(&stateful_tables_map, "stateful table", name,
                 std::move(table));
}

void
P4Objects::add_stateful_timer(StatefulTimer *stateful_timer) {
  stateful_timers_vector.push_back(stateful_timer);
}
...
void
P4Objects::add_register_stack(const std::string &name,
                              std::unique_ptr<RegisterStack> register_stack) {
  add_new_object(&register_stacks, "register_stack", name, std::move(register_stack));
}
...
RegisterStack *
P4Objects::get_register_stack_cfg(const std::string &name) const {
  return get_object(register_stacks, "register_stack", name).get();
}
