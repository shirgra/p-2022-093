...
void
MatchKeyBuilder::get_match_key_params(const PHV &phv, std::vector<MatchKeyParam> *key) const {
  key->reserve(key_input.size());
  ...
  for (const auto &in : key_input) {
    ...
    if (in.mtype == MatchKeyParam::Type::VALID) {
      if (header.is_valid())
        key->emplace_back(in.mtype, header.is_valid() ? "\x01" : "\x00");
    } else {
      // we do not reset all fields to 0 in between packets
      // so I need this hack if the P4 programmer assumed that:
      // field not valid => field set to 0
      ...
      const Field &field = header[in.f_offset];
      if (header.is_valid() || field.is_hidden()) {
        key->emplace_back(in.mtype, std::string(field.get_bytes().data(), field.get_bytes().size()));
      } else {
        key->emplace_back(in.mtype, std::string(field.get_nbytes(), '\x00'));
      }
    }
  }
}
