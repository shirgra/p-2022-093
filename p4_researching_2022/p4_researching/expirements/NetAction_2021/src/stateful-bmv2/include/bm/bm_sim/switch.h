...
class SwitchWContexts : public DevMgr, public RuntimeInterface {
 ...
 public:
  ...
  mt_st_get_entries(cxt_id_t cxt_id,
                 const std::string &table_name) const override {
      return contexts.at(cxt_id).mt_get_entries<FlowTable>(table_name);
  }
  ...
  MatchErrorCode
  mt_st_get_entry(cxt_id_t cxt_id, const std::string &table_name,
               entry_handle_t handle, FlowTable::Entry *entry) const override {
      return contexts.at(cxt_id).mt_get_entry<FlowTable>(
              table_name, handle, entry);
  }
  ...
  MatchErrorCode
  mt_st_get_entry_from_key(cxt_id_t cxt_id, const std::string &table_name,
                        const std::vector<MatchKeyParam> &match_key,
                        FlowTable::Entry *entry,
                        int priority = 1) const override {
    return contexts.at(cxt_id).mt_get_entry_from_key<FlowTable>(
            table_name, match_key, entry, priority);
  }
...
class Switch : public SwitchWContexts {
 public:
  ...
  StatefulTable *get_stateful_table(const std::string &table_name) {
    return get_context(0)->get_stateful_table(table_name);
  }

  std::vector<StatefulTimer*> *get_stateful_timers() {
    return get_context(0)->get_stateful_timers();
  }
