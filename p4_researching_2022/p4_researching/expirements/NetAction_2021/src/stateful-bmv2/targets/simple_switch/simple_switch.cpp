...
packet_id_t SimpleSwitch::timer_packet_id = 10000;
...
class SimpleSwitch::TimerInputBuffer {
public:
  explicit TimerInputBuffer(size_t capacity)
      : capacity(capacity) { }

  int push_front(std::unique_ptr<Packet> &&item) {
    Lock lock(mutex);
    while (queue.size() == capacity) {
      cvar_can_push.wait(lock);
    }
    queue.push_front(std::move(item));
    lock.unlock();
    cvar_can_pop.notify_one();
  ...
  void pop_back(std::unique_ptr<Packet> *pItem) {
    ...
    if (queue.size() > 0) {
      *pItem = std::move(queue.back());
      queue.pop_back();
      lock.unlock();
      cvar_can_push.notify_one();
    }
  }

  bool wait_for(uint64_t granularity) {
    Lock lock(mutex);
    return (bool) cvar_can_pop.wait_for(lock,
...
private:
  ...
  mutable std::mutex mutex;
  mutable std::condition_variable cvar_can_push;
  mutable std::condition_variable cvar_can_pop;
  size_t capacity;
  QueueImpl queue;
};
...
SimpleSwitch::SimpleSwitch(bool enable_swap, port_t drop_port)
  : Switch(enable_swap),
    ...
    timer_input_buffer(new TimerInputBuffer(1024)),
...
SimpleSwitch::start_and_return_() {
  ...
  stateful_timers = this->get_stateful_timers();
  if (stateful_timers) {
    for (size_t i = 0; i < stateful_timers->size(); i++) {
      threads_.push_back(std::thread(&SimpleSwitch::timers_thread, this,
                                      stateful_timers->at(i)->granularity, i));
    }
  }
}
...
void
SimpleSwitch::timers_thread(uint64_t granularity, size_t timer_id) {
  const char data[64]{0};
  struct timespec interval{};

  if (granularity >= 1000000L) {
    interval.tv_nsec = (granularity % 1000000L) * 1000L;
    interval.tv_sec = granularity / 1000000L;
  } else {
    interval.tv_nsec = granularity * 1000L;
    interval.tv_sec = 0;
  }

  while (1) {

    auto tpkt = new_packet_ptr(timer_port, timer_packet_id++,
                               64,
                               bm::PacketBuffer(128, data, 64));


    tpkt->get_phv()->get_header("timer_metadata").mark_valid();
    if (tpkt->get_phv()->has_field("timer_metadata.timer_id")) {
      tpkt->get_phv()->get_field("timer_metadata.timer_id").set_arith(true);
      tpkt->get_phv()->get_field("timer_metadata.timer_id").set(timer_id);
    }

    clock_nanosleep(CLOCK_MONOTONIC, 0, &interval, NULL);

    tpkt->get_phv()->get_field("timer_metadata.ticks_now").set_arith(true);
    tpkt->get_phv()->get_field("timer_metadata.ticks_now")
        .set(get_time_elapsed_us()/granularity);

    // do timer wheel processing
    input_buffer->push_front(InputBuffer::PacketType::TIMER, std::move(tpkt));
...
SimpleSwitch::ingress_thread() {
  ...
  while (1) {
    ...
    // Handle timeout from timers_thread
    if (timer_port == (size_t)packet->get_ingress_port()) {
      phv = packet->get_phv();
      bm::StatefulTable *stateful_table;

      //TODO: enable multiple timer_metadata instances (one for each timer)
      ticks_now = phv->get_field("timer_metadata.ticks_now").get_uint64();

      size_t timer_table_id = phv->get_field("timer_metadata.timer_id")
                                    .get_uint();

      phv->get_field("standard_metadata.ingress_port").set(timer_port);

      stateful_table = this->get_stateful_table(
                          stateful_timers->at(timer_table_id)->table_name);

      stateful_table->apply_timer(packet.get());
      continue;
    }

    phv = packet->get_phv();

    if (phv->has_header("timer_metadata")) {
      phv->get_header("timer_metadata").mark_valid();
      phv->get_field("timer_metadata.ticks_now").set(ticks_now);
    }

    if (phv->has_header("queue_metadata")) {
      phv->get_header("queue_metadata").mark_valid();
      for (unsigned int i = 0; i < nb_egress_threads; i++) {
        std::string qd_string = "queue_metadata.qdepth_" + std::to_string(i);

        phv->get_field(qd_string).set(egress_buffers.size(i));
      }
    }
