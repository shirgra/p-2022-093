    ...
    apply {
        ...
        if (local_metadata.state0 == NEW) {
            if (standard_metadata.ingress_port == INTERNAL) {
                ...
                ports.read(local_metadata.nat_port, (bit<32>) tmp_idx);
                ...
                local_metadata.state0 = FREE_PORT_PICKED_UP;
            } else if (standard_metadata.ingress_port == EXTERNAL) {
                local_metadata.stage1_fk = hdr.tcp.dst_port;
            }
        } else if (local_metadata.state0 == FREE_PORT_PICKED_UP) {
            if (standard_metadata.ingress_port == INTERNAL) {
                local_metadata.stage1_fk = local_metadata.nat_port;
            }
        }
        ...
        if (local_metadata.state1 == NEW) {
            if (standard_metadata.ingress_port == INTERNAL) {
                ...
                local_metadata.state1 = ESTABLISHED;
            } else if (standard_metadata.ingress_port == EXTERNAL) {
                mark_to_drop();
            }
        } else if (local_metadata.state1 == ESTABLISHED) {
            if (standard_metadata.ingress_port == INTERNAL) {
                natIPAddress.read(hdr.ipv4.src_addr, 0);
