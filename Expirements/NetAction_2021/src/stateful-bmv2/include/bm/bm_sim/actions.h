...
struct ActionParam {
  ...
  enum {CONST, FIELD, HEADER, ACTION_DATA, REGISTER_REF, REGISTER_GEN,
        ...
        EXTERN_INSTANCE,
        STRING,
        HEADER_UNION, HEADER_UNION_STACK, PARAMS_VECTOR, REGISTER_STACK} tag;
...
template <> inline
RegisterStack &ActionParam::to<RegisterStack &>(
    ActionEngineState *state) const {
  (void) state;
  assert(tag == ActionParam::REGISTER_STACK);
  return *(register_stack);
}

template <> inline
const RegisterStack &ActionParam::to<const RegisterStack &>(
    ActionEngineState *state) const {
  return ActionParam::to<RegisterStack &>(state);
}
...
class ActionFn :  public NamedP4Object {
 ...
 public:
  ...
  void parameter_push_back_register_stack(RegisterStack *register_stack);
