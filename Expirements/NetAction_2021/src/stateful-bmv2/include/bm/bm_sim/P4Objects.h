...
class P4Objects {
 ...
 public:
  ...
  MatchTableAbstract *get_abstract_state_table(const std::string &name) const {
    return stateful_tables_map.at(name)->get_match_table();
  }
  ...
  StatefulTable *get_stateful_table(const std::string &name) const {
    return stateful_tables_map.at(name).get();
  }

  std::vector<StatefulTimer *> *get_stateful_timers() {
    return &stateful_timers_vector;
  }
 ...
 private:
  ...
  void add_stateful_table(const std::string &name,
                              std::unique_ptr<StatefulTable> table);

  void add_stateful_timer(StatefulTimer *stateful_timer);
  ...
  void add_register_stack(const std::string &name,
                          std::unique_ptr<RegisterStack> register_stack);

  RegisterArray *get_register_array_cfg(const std::string &name) const;

  RegisterStack *get_register_stack_cfg(const std::string &name) const;
  ...
  void init_register_stacks(const Json::Value &root);
 ...
 private:
  ...
  std::unordered_map<std::string, std::unique_ptr<StatefulTable> >
  stateful_tables_map{};

  std::vector<StatefulTimer *> stateful_timers_vector{};
  ...
  // register stacks
  std::unordered_map<std::string, std::unique_ptr<RegisterStack> >
      register_stacks{};
