...
class Meter {
 ...
 public:
  ...
  MeterErrorCode set_ewma_rate(uint64_t window_size);
  ...
  uint64_t execute_ewma(const Packet &pkt, uint64_t current_sample);
 ...
 private:
  ...
  struct EwmaRate {
    uint64_t ewma_res{};
    uint64_t current_slot_samples;
    uint64_t last_ts{};
    uint64_t current_slot_ts{};
    uint64_t window_size; //in us
    bool configured;
  };
...
class MeterArray : public NamedP4Object {
 ...
 public:
  ...
  uint64_t execute_ewma_meter(const Packet &pkt, size_t idx, uint64_t current_sample);
  ...
  MeterErrorCode set_ewma_rates(uint64_t window_size);
