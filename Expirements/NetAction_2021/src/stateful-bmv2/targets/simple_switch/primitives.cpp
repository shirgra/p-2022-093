...
class execute_ewma_meter
    : public ActionPrimitive<MeterArray &, const Data &, Field &, const Data &> {
  void operator ()(MeterArray &meter_array, const Data &idx, Field &dst, const Data &sample) {
    auto i = idx.get_uint();
#ifndef NDEBUG
    if (i >= meter_array.size()) {
      BMLOG_ERROR_PKT(get_packet(),
...
#endif  // NDEBUG
    uint64_t result = meter_array.execute_ewma_meter(get_packet(), i, sample.get_uint64());
    dst.set(result);
    BMLOG_TRACE_PKT(get_packet(),
                    "Updated meter '{}' at index {},"
                    " assigning dest field the result {}",
                    meter_array.get_name(), i, result);
  }
};

REGISTER_PRIMITIVE(execute_ewma_meter);
...
class register_stack_pop
    : public ActionPrimitive<Field &, RegisterStack &> {
  void operator ()(Field &dst, RegisterStack &reg_stack) {
    if (!reg_stack.is_empty()) {
      dst.set(reg_stack.pop());
      BMLOG_DEBUG("Popped {} from register stack", dst.get_uint())
    } else {
      BMLOG_ERROR("Register stack is empty, doing nothing");
    }
  }
};

REGISTER_PRIMITIVE(register_stack_pop);

class register_stack_push
    : public ActionPrimitive<Field &, RegisterStack &> {
  void operator ()(Field &dst, RegisterStack &reg_stack) {
    if (!reg_stack.is_full()) {
      reg_stack.push(dst);
      BMLOG_DEBUG("pushed {} to register stack", dst.get_uint())
    } else {
      BMLOG_ERROR("Register stack is full, doing nothing");
    }
  }
};

REGISTER_PRIMITIVE(register_stack_push);
