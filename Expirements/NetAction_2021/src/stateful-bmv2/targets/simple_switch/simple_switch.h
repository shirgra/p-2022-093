...
class SimpleSwitch : public Switch {
 ...
 private:
  ...
  static packet_id_t timer_packet_id;

  class MirroringSessions;

  class InputBuffer;
  class TimerInputBuffer;
  ...
  enum PktInstanceType {
    ...
    PKT_INSTANCE_TYPE_TIMEOUT_EVENT
  };
 ...
 private:
  std::vector<bm::StatefulTimer*> *stateful_timers{};
  uint64_t ticks_now;
 ...
 private:
  ...
  [[noreturn]] void timers_thread(uint64_t granularity, size_t timer_id);
 ...
 private:
  port_t drop_port;
  port_t timer_port = 200;
  std::vector<std::thread> threads_;
  std::unique_ptr<InputBuffer> input_buffer;
  std::unique_ptr<TimerInputBuffer> timer_input_buffer;
