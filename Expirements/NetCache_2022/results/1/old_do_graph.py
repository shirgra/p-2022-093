...
fig, ax = plt.subplots()
for res, name in zip([res_cont, res_agg, res_Tor], ["Controller Switch", "Aggregation Switches", "TOR Switches"]):
    # build slices
...
fig, ax = plt.subplots()
for res, name, color, linestyle in zip([res_cont, res_agg, res_Tor], ["Controller Switch", "Aggregation Switches", "TOR Switches"], ["Black", "Cyan", "Green"], ['--', '-', '-.']):
    # build slices
    y_points = [0 for i in list(range(len(x_points)))]
    # fill points for switch
    for hit in res:
...
plt.savefig(title + "______func_of_pkts.png")
