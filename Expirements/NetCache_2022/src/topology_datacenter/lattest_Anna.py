...
s4_hit_threshold = {}  # {1: 0}  -> { policy_id: threshold } 
s5_hit_threshold = {}  # {1: 0}  -> { policy_id: threshold } 
s6_hit_threshold = {}  # {1: 0}  -> { policy_id: threshold } 
######################################################################################################################################## Class CacheSwitch
...
def handle_hit_pkt(pkt,switch):

    global s4_hit_threshold, s5_hit_threshold, s6_hit_threshold
    # parse packet 
    source_host_ip      = pkt[IP].src # recognize the switch src
    lookup_ip_request   = pkt[IP].dst # the request for an unknown destination
    sys.stdout.flush()

    if switch.name_str == 's4':
        switch = s4
        switch_name = "s4"
        switch_cache = s4_cache
        switch_hit_threshold = s4_hit_threshold
    if switch.name_str == 's5':
        switch = s5
        switch_name = "s5"
        switch_cache = s5_cache
        switch_hit_threshold = s5_hit_threshold
    if switch.name_str == 's6':
        switch = s6
        switch_name = "s6"
        switch_cache = s6_cache
        switch_hit_threshold = s6_hit_threshold
    ...
    if controller_answer != -1:
        ...
        # parse answer
        address   = policy_rules[controller_answer][0]                 # get rule address
        mask      = policy_rules[controller_answer][1]                 # get rule mask
        threshold = switch_hit_threshold.get(controller_answer, 0)  # get rule treshold
        ...
        if threshold > HIT_THRESHOLD:
            ...
            # check if already in switch cache
                if longestCommonPrefix(address,"192.240.0.0") > 12:
                # to which cache enter the rule
                    switch = s4
                    switch_name = "s4"
                    switch_cache = s4_cache
                ...
                #TODO algorithem to match s1,s2,s3 also XXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXX
                #TODO return "switch_hit_threshold" back to "sx_hit_threshold" maybe in the end XXXXXXXXXXXXXXXXXXXXXXXXXXX
                ...
                # cache is not full yet - just write:
                    ...
                    #TODO change action="MyIngress.drop" to something correct XXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXX
                ...
                # cache is full  - delete LRU then write
            ...
            # reset threshold
            switch_hit_threshold[controller_answer] = 0
        else:
            # update threshold
            switch_hit_threshold[controller_answer] = threshold + 1
